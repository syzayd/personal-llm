...
| **v1.0 Personal assistant** (integration slice done) | 1-2 real integrations (Gmail/Drive via existing MCP - done), packaging, polish (remaining) | `integrations/` (credential-free ingestion of externally-fetched content, idempotent by doc_id), `cli ingest-external`, `POST /integrations/sync` | Real inbox/drive-aware answers - proven live with real Gmail threads + a real Drive doc; daily-usable packaging still open |
